...
import es1.customExceptions.ParseStockPricesRecordException;
import es1.customWritables.SelectedFieldsOfStockPricesRecordWritable;
import es1.domainObjects.StockPricesRecord;
...
public class PrimoMapper extends Mapper<LongWritable, Text, Text, SelectedFieldsOfStockPricesRecordWritable> {
    ...
    private Date minDate = simpleDateFormat.parse("2008-01-01");
    ...
    public void map(LongWritable key, Text value, Context context) {
        ...
        //sono interessato solamente ai record con anno >=2008 e <=2018
        if(stockPricesRecord.getDate().compareTo(this.minDate) < 0 || stockPricesRecord.getDate().compareTo(this.maxDate) > 0) {
            //log.info("Scartata linea con anno non compreso tra 2008 e 2018: " + value.toString());
            return;
        }
        ...
        SelectedFieldsOfStockPricesRecordWritable selectedFieldsOfStockPricesRecordWritable = new SelectedFieldsOfStockPricesRecordWritable(
                ...
                stockPricesRecord.getVolume(),
                this.simpleDateFormat.format(stockPricesRecord.getDate())
        );

        //log.info("scrivo la coppia: " + tickerWritable.toString() + " - " + Arrays.toString(selectedFields.toStrings()));
